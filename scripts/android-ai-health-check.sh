...
# ---------------------------------------------------------------------------
echo ""
echo "[1/10] Checking version sync for release v1.8.28..."
...
if [[ "$pkg_version" != "1.8.28" ]]; then
  fail_msg "package.json version is $pkg_version, expected 1.8.28"
else
  ok "package.json version: $pkg_version"
fi

if [[ "$tauri_version" != "1.8.28" ]]; then
  fail_msg "tauri.conf.json version is $tauri_version, expected 1.8.28"
else
  ok "tauri.conf.json version: $tauri_version"
fi

if [[ "$cargo_version" != "1.8.28" ]]; then
  fail_msg "Cargo.toml version is $cargo_version, expected 1.8.28"
else
...
if [[ "$fail" -eq 0 && "$warn" -eq 0 ]]; then
  ...
  echo "All systems operational for release v1.8.28"
  exit 0
elif [[ "$fail" -eq 0 ]]; then
