...
void ClarityOp::ApplyGPU(std::shared_ptr<ImageBuffer>) {
  // Handled by the GPU fused pipeline's neighbor stage (edit_pipeline_detail.cl:
  // opencl_apply_clarity). Use the pipeline for GPU rendering.
}
...
void ClarityOp::SetGlobalParams(OperatorParams& params) const {
  ...
  BuildGaussianKernel(usm_radius_, 48, params.clarity_gaussian_tap_count_,
