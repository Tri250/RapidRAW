...
export const PANEL_ICONS: Record<Panel, LucideIcon> = {
  ...
  [Panel.Settings]: SettingsIcon,
};
const PANEL_TITLES: Record<Panel, string> = {
  [Panel.Metadata]: 'editor.switcher.tooltips.info',
  [Panel.Adjustments]: 'editor.switcher.tooltips.adjust',
  [Panel.Color]: 'editor.switcher.tooltips.color',
  [Panel.Portrait]: 'editor.switcher.tooltips.portrait',
  ...
  [Panel.Settings]: 'editor.switcher.tooltips.settings',
};
function PanelTab({ panel, region, side }: { panel: Panel; region: PanelRegion; side: 'left' | 'right' }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      data-tooltip={t(PANEL_TITLES[panel] as any)}
...
export default function PanelSwitcher({
  ...
  const panels: Panel[] = panelLayout[region];
  ...
  useDndMonitor({
    onDragMove(event) {
      ...
      const tabs = Array.from(containerRef.current.querySelectorAll('[data-layout-tab]')).filter(
        (tab: Element) => tab.getAttribute('data-tab-id') !== event.active.id,
      ) as HTMLElement[];
