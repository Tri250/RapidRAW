...
export default function CurveGraph({
  ...
  useEffect(() => {
    const handleMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (!isParametricMode && draggingIndexRef.current !== null) {
        ...
        const te = 'touches' in e ? (e as TouchEvent) : null;
        const clientX = te ? te.touches[0].clientX : me.clientX;
        const clientY = te ? te.touches[0].clientY : me.clientY;
  ...
  const handlePointStart = (e: React.MouseEvent | React.TouchEvent, index: number) => {
    if (isParametricMode || (!('touches' in e) && e.button === 2)) return;
  ...
  const handleContainerStart = (e: React.MouseEvent | React.TouchEvent) => {
    if (isParametricMode || (!('touches' in e) && e.button !== 0) || (e.target as HTMLElement).tagName === 'circle')
      return;
    onDragStateChange?.(true);
    ...
    const te = 'touches' in e ? (e as React.TouchEvent) : null;
    const clientX = te ? te.touches[0].clientX : me.clientX;
    const clientY = te ? te.touches[0].clientY : me.clientY;
    const rect = svg.getBoundingClientRect();
      ...
      <div className="flex items-center justify-between gap-2 mb-2 mt-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {Object.keys(channelConfig).map((channel: string) => {
            ...
            const channelLabel = t(`adjustments.curves.channels.${channel}` as unknown as TemplateStringsArray);
            return (
              <button
                key={channel}
                ...
                title={t('adjustments.curves.channelTitle', { channel: channelLabel as string })}
              >
                <Text variant={TextVariants.small} color={TextColors.primary} weight={TextWeights.bold}>
                  {(channelLabel as string).charAt(0).toUpperCase()}
                </Text>
              </button>
            );
          })}
            ...
            {!isParametricMode &&
              activePoints.map((p: Coord, i: number) => (
                <circle
                  ...
                  fill={color}
                  key={i}
                  onMouseDown={(e: React.MouseEvent) => handlePointStart(e, i)}
                  onTouchStart={(e: React.TouchEvent) => handlePointStart(e, i)}
              ...
              <div className="flex flex-col gap-2">
                <Slider
                  ...
                  onChange={(e: { target: { value: string } }) =>
                    updateParametricValue('whiteLevel', parseFloat(e.target.value))
                  }
                ...
                <Slider
                  ...
                  onChange={(e: { target: { value: string } }) =>
                    updateParametricValue('highlights', parseFloat(e.target.value))
                  }
                  onDragStateChange={onDragStateChange}
                />
                <Slider
                  ...
                  onChange={(e: { target: { value: string } }) =>
                    updateParametricValue('lights', parseFloat(e.target.value))
                  }
                  onDragStateChange={onDragStateChange}
                />
                <Slider
                  ...
                  onChange={(e: { target: { value: string } }) =>
                    updateParametricValue('darks', parseFloat(e.target.value))
                  }
                  onDragStateChange={onDragStateChange}
                />
                <Slider
                  ...
                  onChange={(e: { target: { value: string } }) =>
                    updateParametricValue('shadows', parseFloat(e.target.value))
                  }
                  onDragStateChange={onDragStateChange}
                />
                <Slider
                  ...
                  onChange={(e: { target: { value: string } }) =>
                    updateParametricValue('blackLevel', parseFloat(e.target.value))
                  }
                  onDragStateChange={onDragStateChange}
                />
              </div>
