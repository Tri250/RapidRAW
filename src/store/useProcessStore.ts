...
export const useProcessStore = create<ProcessState>((set, get) => {
  const timeouts = {
    export: null as ReturnType<typeof setTimeout> | null,
    import: null as ReturnType<typeof setTimeout> | null,
    copy: null as ReturnType<typeof setTimeout> | null,
    paste: null as ReturnType<typeof setTimeout> | null,
  };
  ...
  return {
    ...
    setProcess: (updater) => {
      ...
      const state = get();
      if (state.isCopied) {
        if (timeouts.copy) clearTimeout(timeouts.copy);
        timeouts.copy = setTimeout(() => set({ isCopied: false }), 1000);
      }
      if (state.isPasted) {
        if (timeouts.paste) clearTimeout(timeouts.paste);
        timeouts.paste = setTimeout(() => set({ isPasted: false }), 1000);
      }
    },
    ...
    setExportState: (updater) => {
      ...
      if (timeouts.export) clearTimeout(timeouts.export);

      if ([Status.Success, Status.Error, Status.Cancelled].includes(status)) {
        timeouts.export = setTimeout(() => {
          set((prev) => ({
            exportState: {
              ...prev.exportState,
              status: Status.Idle,
    ...
    setImportState: (updater) => {
      ...
      if (timeouts.import) clearTimeout(timeouts.import);

      if ([Status.Success, Status.Error, Status.Cancelled].includes(status)) {
        timeouts.import = setTimeout(() => {
          set((prev) => ({
            importState: {
              ...prev.importState,
              status: Status.Idle,
